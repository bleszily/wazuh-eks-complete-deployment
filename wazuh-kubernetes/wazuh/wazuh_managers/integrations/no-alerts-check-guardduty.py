...
# send alert if no guardduty alerts are present in the last 2 days
...
for i in range(0, days_back):
    ...
    if count != 0:
        json_response = "{\"task\":\"guardduty_daily_alerts_check\", \"result\":\"SUCCESS\"}"
        print(json_response)
        exit()

json_response = "{\"task\":\"guardduty_daily_alerts_check\", \"result\":\"ERROR\"}"
print(json_response)
